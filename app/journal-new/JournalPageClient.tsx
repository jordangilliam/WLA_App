...
export default function JournalPageClient() {
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredObservations.map((obs) => {
      ...
      const prompts = obs.reflection_prompts || {};
      return [
        ...
        obs.mood || '',
        (obs.tags || []).join('; '),
        (prompts.conditions || '').replace(/"/g, '""'),
        (prompts.habitat || '').replace(/"/g, '""'),
        (prompts.phenology || '').replace(/"/g, '""'),
        (prompts.action || '').replace(/"/g, '""'),
        (prompts.feelings || '').replace(/"/g, '""'),
      ];
    });
  ...
  const handleSaveReflection = async (
    ...
    try {
      ...
      const updatedObservation = result.observation as Observation;

      setObservations((prev) =>
        prev.map((obs) => (obs.id === id ? { ...obs, ...updatedObservation } : obs))
      );
    } catch (error) {
