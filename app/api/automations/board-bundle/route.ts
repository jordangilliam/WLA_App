...
export async function POST(req: NextRequest){
  ...
  if (gtok){
    ...
    const board = await gdEnsureFolder("Board", admin, gtok);
    const q = await gdEnsureFolder(qp, board, gtok);
    const mdu = await gdUploadString(`bundle-${qp}.md`, "text/markdown", md, q, gtok);
    const cs = await gdUploadString(`bundle-${qp}.csv`, "text/csv", csv, q, gtok);
  ...
  if (mtok){
    ...
    const board = await odEnsureChildFolder(admin, "Board", mtok);
    const q = await odEnsureChildFolder(board, qp, mtok);
    const mdu = await odUploadString(q, `bundle-${qp}.md`, "text/markdown", md, mtok);
    const cs = await odUploadString(q, `bundle-${qp}.csv`, "text/csv", csv, mtok);
